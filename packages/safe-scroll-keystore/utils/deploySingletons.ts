...
import { AddressLike, getCreate2Address, keccak256, parseUnits, ZeroHash } from 'ethers'
...
export default async function deploySingletons(deployer: SignerWithAddress) {
  ...
  const safeMultiSendAddress = await deploySingleton(factoryAddress, ArtifactMultiSend.bytecode, deployer)
  const safeMultiSendCallOnlyAddress = await deploySingleton(factoryAddress, ArtifactMultiSendCallOnly.bytecode, deployer)
  const safeFallbackHandlerAddress = await deploySingleton(factoryAddress, ArtifactFallbackHandler.bytecode, deployer)
  const safeSignMessageLibAddress = await deploySingleton(factoryAddress, ArtifactSignMessageLib.bytecode, deployer)
  const safeCreateCallAddress = await deploySingleton(factoryAddress, ArtifactCreateCall.bytecode, deployer)
  const safeSimulateTxAccessorAddress = await deploySingleton(factoryAddress, ArtifactSimulateTxAccessor.bytecode, deployer)
